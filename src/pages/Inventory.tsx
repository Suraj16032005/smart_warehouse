                  ...
                  {filtered.map(r => {
                    ...
                    return (
                      <tr key={r.id} className="border-t border-foreground/10 hover:bg-paper-2/50">
                        ...
                        <td className="px-4 py-3 font-mono text-xs">{r.location || "—"}</td>
                        <td className="px-4 py-3 font-mono text-[10px] text-muted-foreground">{new Date(r.updated_at).toLocaleString()}</td>
        ...
        <DialogContent className="rounded-none bg-card border-foreground/30 max-w-md">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="font-mono text-[10px] tracking-[0.2em] uppercase">Low threshold</Label>
                <Input type="number" min={0} value={form.low_stock_threshold} onChange={e => setForm({ ...form, low_stock_threshold: Number(e.target.value) })}
                  className="rounded-none border-foreground/30 focus-visible:ring-0 focus-visible:border-primary font-mono" />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="font-mono text-[10px] tracking-[0.2em] uppercase">Location</Label>
              <Input value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} maxLength={60} placeholder="e.g. Aisle A3 / Shelf 2"
